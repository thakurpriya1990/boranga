<template lang="html">
    <div id="occurrenceCombineContacts">
        <datatable ref="contacts_datatable" :id="panelBody" :dtOptions="contacts_options"
    ...
    methods: {
        getSelectedContactNames: function () {
            ...
            vm.selectedKeyContacts.forEach(contact => {
                if (vm.combineKeyContactIds.includes(contact.id) && !names.includes(contact.contact_name)) {
        ...
        adjust_table_width: function () {
            if (this.$refs.contacts_datatable !== undefined) { this.$refs.contacts_datatable.vmDataTable.columns.adjust().responsive.recalc() };
            helpers.enablePopovers();
        },
        removeContact: function(id) {
            let vm=this;   
            vm.combineKeyContactIds.splice(vm.combineKeyContactIds.indexOf(id), 1);
        ...
        addEventListeners:function (){
            let vm=this;     
            vm.$refs.contacts_datatable.vmDataTable.on('change', 'input[data-contact-checkbox]', function(e) {
                e.preventDefault();
                var id = parseInt($(this).attr('data-contact-checkbox'));
                if($(this).prop('checked')) {
                    vm.addContact(id);
                    vm.selectedKeyContacts.forEach(contact=> {
                        let checkbox = vm.$refs.contacts_datatable.vmDataTable.$("#"+contact.id);
                        if (id != checkbox.attr('data-contact-checkbox') && checkbox.attr('contact-name') == $(this).attr('contact-name')) {
            ...
            vm.$refs.contacts_datatable.vmDataTable.on('childRow.dt', function (e, settings) {
                helpers.enablePopovers();
            });
        }
    },
