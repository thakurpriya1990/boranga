<template lang="html">
    <div id="occurrenceCombine">
        <modal transition="modal fade" :title="'Combine Occurrences'" :extraLarge="true" :full="true" :showOK="false"
            cancel-text="Cancel" @cancel="close()">
            ...
            <!--OCC Selection Table-->
            <div class="row mb-3">
                <OccurrenceCombineSelect :selectedOccurrences="selectedOccurrences"
                    :selectedOccurrenceIds="selectedOccurrenceIds" :mainOccurrenceId="main_occurrence_obj.id" />
            </div>

            <template v-if="occ_combine_data.combine_ids.length >= 2">

                <alert type="primary" class="d-flex align-items-center py-1 my-4">
                    <div class="float-start pe-3"><span class="align-middle"><i
                                class="bi bi-info-circle-fill text-primary fs-4"></i></span></div>
                    <ul class="list-group list-group-flush">
                        <li class="list-group-item bg-transparent">Browse the contents of
                            each to tab to select the data you would like to combine into Occurrence {{
                                main_occurrence_obj.occurrence_number }}</li>
                        <li class="list-group-item bg-transparent">Once you are
                            finished click the 'Finalise Combine' tab to completed the process.</li>
                    </ul>
                </alert>
                ...
                <div class="row mb-3">
                    <div class="col-md-12">
                        <ul id="pills-tab" class="nav nav-pills" role="tablist">
                            <li class="nav-item">
                                <a id="pills-occurrence-tab" class="nav-link active" data-bs-toggle="pill"
                                    :href="'#' + occurrenceBody" role="tab" :aria-controls="occurrenceBody"
                                    aria-selected="true">
                                    Occurrence
                                </a>
                            </li>
                            <li class="nav-item">
                                <a id="pills-location-tab" class="nav-link" data-bs-toggle="pill"
                                    :href="'#' + locationBody" role="tab" aria-selected="false">
                                    Location
                                </a>
                            </li>
                            <li class="nav-item">
                                <a id="pills-habitat-tab" class="nav-link" data-bs-toggle="pill"
                                    :href="'#' + habitatBody" role="tab" aria-selected="false">
                                    Habitat
                                </a>
                            </li>
                            <li class="nav-item">
                                <a id="pills-observation-tab" class="nav-link" data-bs-toggle="pill"
                                    :href="'#' + observationBody" role="tab" aria-selected="false">
                                    Observation
                                </a>
                            </li>
                            <li class="nav-item">
                                <a id="pills-documents-tab" class="nav-link" data-bs-toggle="pill"
                                    :href="'#' + documentBody" role="tab" aria-selected="false">
                                    Documents
                                </a>
                            </li>
                            <li class="nav-item">
                                <a id="pills-threats-tab" class="nav-link" data-bs-toggle="pill"
                                    :href="'#' + threatBody" role="tab" aria-selected="false">
                                    Threats
                                </a>
                            </li>
                            <li class="nav-item">
                                <a id="pills-finalise-combine-tab" class="nav-link" data-bs-toggle="pill"
                                    href="'#finalise-occurrence-combine" role="tab" aria-selected="false"><i
                        ...
                        <div id="pills-tabContent" class="tab-content">
                            <div :id="occurrenceBody" class="tab-pane fade show active" role="tabpanel"
                                ...
                                <FormSection :formCollapse="false" label="Occurrence" Index="combine_occurrence">
                                    <div class="row mb-3">
                                        ...
                                        <div class="col-sm-9">
                                            <textarea disabled class="form-control" rows="1"
                                                v-model="main_occurrence_obj.occurrence_name" />
                                        </div>
                                    </div>
                                    <div v-if="main_occurrence_obj.group_type == 'flora' || main_occurrence_obj.group_type == 'fauna'"
                                        class="row mb-3">
                                        <label for="" class="col-sm-3 control-label">Scientific Name:</label>
                                        <div class="col-sm-9">
                                            <textarea disabled class="form-control" rows="1"
                                                v-model="main_occurrence_obj.scientific_name" />
                                        </div>
                                    </div>
                                    <div v-else class="row mb-3">
                                        ...
                                        <div class="col-sm-9">
                                            <textarea disabled class="form-control" rows="1"
                                                v-model="main_occurrence_obj.community_name" />
                                        </div>
                                    </div>
                                    <div class="row mb-3">
                                        <label for="" class="col-sm-3 control-label">Occurrence Source:</label>
                                        <div class="col-sm-9">
                                            <select id="combine_occurrence_source" ref="combine_occurrence_source"
                                                class="form-select" v-model="occ_combine_data.occurrence_source"
                                                :key="occ_form_key">
                                                <option v-for="occurrence in selectedOccurrences" :value="occurrence.id"
                                                    ...
                                                    <span v-for="source in occurrence.occurrence_source">
                                                        <span v-for="occ_source in occurrence_source_list">
                                                            <span v-if="occ_source[0] == source">{{ occ_source[1]
                                                                }}</span>
                                                        </span>
                                                        <span
                                                            v-if="source != occurrence.occurrence_source[occurrence.occurrence_source.length - 1]">,
                                                        </span>
                                                    </span>
                                                </option>
                                            </select>
                                    ...
                                    <div class="row mb-3">
                                        ...
                                        <div class="col-sm-9">
                                            <select id="combine_wild_status" ref="combine_wild_status"
                                                class="form-select" v-model="occ_combine_data.wild_status"
                                                :key="occ_form_key">
                                                <option v-for="occurrence in selectedOccurrences" :value="occurrence.id"
                                                    v-bind:key="occurrence.id">
                                                    {{ occurrence.occurrence_number }}:
                                                    <span v-for="wild_status in wild_status_list">
                                                        <span v-if="wild_status.id == occurrence.wild_status">{{
                                                            wild_status.name }}</span>
                                                    </span>
                                                </option>
                                            </select>
                                        </div>
                                    ...
                                    <div class="row mb-3">
                                        ...
                                        <div class="col-sm-9">
                                            <select id="combine_review_due_date" ref="combine_review_due_date"
                                                class="form-select" v-model="occ_combine_data.review_due_date"
                                                :key="occ_form_key">
                                                <option v-for="occurrence in selectedOccurrences" :value="occurrence.id"
                                                    v-bind:key="occurrence.id">
                                ...
                                <FormSection :formCollapse="true" label="Key Contacts" Index="combine_keyContacts"
                                    ...
                                    <div class="row mb-3">
                                        <OccurrenceCombineContacts :selectedKeyContacts="key_contacts"
                                            :combineKeyContactIds="occ_combine_data.combine_key_contact_ids"
                                            :mainOccurrenceId="main_occurrence_obj.id" :key="contact_table_key"
                            ...
                            <div :id="locationBody" class="tab-pane fade" role="tabpanel"
                                ...
                                <!--Location Form-->
                                <FormSection :formCollapse="true" label="Location"
                                    :subtitle="' - OCC' + occ_combine_data.chosen_location_section"
                                    Index="combine_location">
                                    <div class="row mb-3">
                                        <OccurrenceCombineSelect
                                            :occ_chosen_section="occ_combine_data.chosen_location_section"
                                            :section_type="'location'" :selectedOccurrences="selectedOccurrences"
                                            :selectedOccurrenceIds="selectedOccurrenceIds"
                                            :mainOccurrenceId="main_occurrence_obj.id"
                                            @updateChosenSection="updateChosenSection" />
                                    </div>
                                </FormSection>
                                <FormSection :formCollapse="true" label="Sites" Index="combine_sites"
                                    @toggle-collapse="toggleSites">
                                    <div class="row mb-3">
                                        <OccurrenceCombineSites :selectedSites="sites"
                                            :combineSiteIds="occ_combine_data.combine_site_ids"
                                            :mainOccurrenceId="main_occurrence_obj.id" :key="site_table_key"
                                            ref="sites_section" />
                                    </div>
                                </FormSection>
                                <FormSection :formCollapse="true" label="Tenures" Index="combine_tenures"
                                    @toggle-collapse="toggleTenures">
                                    <div class="row mb-3">
                                        <OccurrenceCombineTenures :selectedTenures="tenures"
                                            :combineTenureIds="occ_combine_data.combine_tenure_ids"
                                            :mainOccurrenceId="main_occurrence_obj.id" :key="tenure_table_key"
                                            ref="tenures_section" />
                                    </div>
                                </FormSection>
                            </div>
                            <div :id="habitatBody" class="tab-pane fade" role="tabpanel"
                                aria-labelledby="pills-habitat-tab">
                                <!--Habitat Composition Form-->
                                <FormSection :formCollapse="true" label="Habitat Composition"
                                    :subtitle="' - OCC' + occ_combine_data.chosen_habitat_composition_section"
                                    Index="combine_habitat_composition">
                                    <div class="row mb-3">
                                        <OccurrenceCombineSelect
                                            :occ_chosen_section="occ_combine_data.chosen_habitat_composition_section"
                                ...
                                <FormSection :formCollapse="true" label="Habitat Condition"
                                    :subtitle="' - OCC' + occ_combine_data.chosen_habitat_condition_section"
                                    Index="combine_habitat_condition">
                                    <div class="row mb-3">
                                        <OccurrenceCombineSelect
                                            :occ_chosen_section="occ_combine_data.chosen_habitat_condition_section"
                                ...
                                <FormSection :formCollapse="true" label="Vegetation Structure"
                                    :subtitle="' - OCC' + occ_combine_data.chosen_vegetation_structure_section"
                                    Index="combine_vegetation_structure">
                                    <div class="row mb-3">
                                        <OccurrenceCombineSelect
                                            :occ_chosen_section="occ_combine_data.chosen_vegetation_structure_section"
                                ...
                                <!--Fire History Form-->
                                <FormSection :formCollapse="true" label="Fire History"
                                    :subtitle="' - OCC' + occ_combine_data.chosen_fire_history_section"
                                    Index="combine_fire_history">
                                    <div class="row mb-3">
                                        <OccurrenceCombineSelect
                                            :occ_chosen_section="occ_combine_data.chosen_fire_history_section"
                                            :section_type="'fire_history'" :selectedOccurrences="selectedOccurrences"
                                            :selectedOccurrenceIds="selectedOccurrenceIds"
                                            :mainOccurrenceId="main_occurrence_obj.id"
                                            @updateChosenSection="updateChosenSection" />
                                    </div>
                                ...
                                <FormSection :formCollapse="true" label="Associated Species"
                                    :subtitle="' - OCC' + occ_combine_data.chosen_associated_species_section"
                                    Index="combine_associated_species">
                                    <div class="row mb-3">
                                        <OccurrenceCombineSelect
                                            :occ_chosen_section="occ_combine_data.chosen_associated_species_section"
                            ...
                            <div :id="observationBody" class="tab-pane fade" role="tabpanel"
                                ...
                                <FormSection :formCollapse="true" label="Observation Details"
                                    :subtitle="' - OCC' + occ_combine_data.chosen_observation_detail_section"
                                    Index="combine_observation_details">
                                    <div class="row mb-3">
                                        <OccurrenceCombineSelect
                                            :occ_chosen_section="occ_combine_data.chosen_observation_detail_section"
                                ...
                                <FormSection v-if="main_occurrence_obj.group_type == 'fauna'" :formCollapse="true"
                                    label="Animal Observation"
                                    :subtitle="' - OCC' + occ_combine_data.chosen_animal_observation_section"
                                    Index="combine_animal_observation">
                                    <div class="row mb-3">
                                        <OccurrenceCombineSelect
                                            :occ_chosen_section="occ_combine_data.chosen_animal_observation_section"
                                ...
                                <FormSection v-if="main_occurrence_obj.group_type == 'flora'" :formCollapse="true"
                                    label="Plant Count"
                                    :subtitle="' - OCC' + occ_combine_data.chosen_plant_count_section"
                                    Index="combine_plant_count">
                                    <div class="row mb-3">
                                        <OccurrenceCombineSelect
                                            :occ_chosen_section="occ_combine_data.chosen_plant_count_section"
                                            :section_type="'plant_count'" :selectedOccurrences="selectedOccurrences"
                                            :selectedOccurrenceIds="selectedOccurrenceIds"
                                            :mainOccurrenceId="main_occurrence_obj.id"
                                            @updateChosenSection="updateChosenSection" />
                                    </div>
                                ...
                                <!--Identification Form-->
                                <FormSection :formCollapse="true" label="Identification"
                                    :subtitle="' - OCC' + occ_combine_data.chosen_identification_section"
                                    Index="combine_identification">
                                    <div class="row mb-3">
                                        <OccurrenceCombineSelect
                                            :occ_chosen_section="occ_combine_data.chosen_identification_section"
                                            :section_type="'identification'" :selectedOccurrences="selectedOccurrences"
                                            :selectedOccurrenceIds="selectedOccurrenceIds"
                                            :mainOccurrenceId="main_occurrence_obj.id"
                                            @updateChosenSection="updateChosenSection" />
                                    </div>
                            ...
                            <div :id="documentBody" class="tab-pane fade" role="tabpanel"
                                ...
                                <FormSection :formCollapse="false" label="Documents" Index="combine_documents"
                                    ...
                                    <div class="row mb-3">
                                        <OccurrenceCombineDocuments :selectedDocuments="documents"
                                            :combineDocumentIds="occ_combine_data.combine_document_ids"
                                            :mainOccurrenceId="main_occurrence_obj.id" :key="document_table_key"
                                            ref="documents_section" />
                                    </div>
                                </FormSection>
                            </div>
                            <div :id="threatBody" class="tab-pane fade" role="tabpanel"
                                aria-labelledby="pills-threats-tab">
                                <!--Threats Table-->
                                <FormSection :formCollapse="false" label="Threats" Index="combine_threats">
                                    <div class="row mb-3">
                                        <OccurrenceCombineThreats :selectedThreats="threats"
                                            :combineThreatIds="occ_combine_data.combine_threat_ids"
                                            :mainOccurrenceId="main_occurrence_obj.id" :key="threat_table_key"
                                            ref="threats_section" />
                                    </div>
                                </FormSection>
                            </div>
                            <div id="finalise-occurrence-combine" class="tab-pane fade" role="tabpanel"
                                aria-labelledby="pills-occurrence-combine-tab">
                                <FormSection :formCollapse="false" label="Finalise Combine" Index="finalise_combine">
                                    <div class="row mb-3">
                                        <div class="col-sm-12">
                                            <button type="button" class="btn btn-primary" @click="ok()"><i
                                                    class="bi bi-check2-circle"></i> Finalise Combine</button>
                                        </div>
                                    </div>
                                </FormSection>
                            </div>
