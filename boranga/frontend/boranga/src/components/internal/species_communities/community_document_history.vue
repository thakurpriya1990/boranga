<template lang="html">
    <div id="communityDocumentHistory">
        <modal
            transition="modal fade"
            :title="'Community C'+ communityId +' - Document D' + documentId + ' - History ' "
            :large="true"
            :full="true"
            :showOK="false"
            cancel-text="Close"
                            ...
                            <div class="row">
                                <div v-if="documentId" class="col-lg-12">
                                    ...
                                    <DisplayHistory
                                        ...
                                        :primary_model="'CommunityDocument'"
                                    />
                                    </div>
                                </div>
                            </div>
...
export default {
    ...
    computed: {
        ...
        column_data: function () {
            return {
                ...
                render: function (row, type, full) {
                    return full.data.data.communitydocument;
                },
                name: 'data',
            };
        },
        ...
        column_id: function () {
            return {
                // 1. ID
                data: 'data.data.communitydocument.pk',
                orderable: false,
                searchable: false,
                visible: false,
                render: function (row, type, full) {
                    return full.data.communitydocument.pk;
                },
                name: 'id',
            };
        },
        column_number: function () {
            return {
                // 2. Number
                data: 'data.data.communitydocument.fields.document_number',
                defaultContent: '',
                orderable: false,
                searchable: false, 
                visible: true,
        ...
        column_category: function () {
            return {
                data: 'data.data.communitydocument.fields.document_category', 
                ...
                visible: true,
                render: function (row, type, full) {
                    if(full.data.communitydocument.fields.document_category) {
                        return full.data.communitydocument.fields.document_category.document_category_name;
                    } else {
                        return ''
                    }
                },
        ...
        column_sub_category: function () {
            return {
                ...
                data: 'data.data.communitydocument.fields.document_sub_category',
                defaultContent: '',
                orderable: false,
                searchable: false, 
                visible: true,
                render: function (row, type, full) {
                    if(full.data.communitydocument.fields.document_sub_category) {
                        return full.data.communitydocument.fields.document_sub_category.document_sub_category_name;
                    } else {
                        return ''
                    }
                },
        ...
        column_file: function () {
            return {
                // 3. File
                data: 'data.data.communitydocument.fields.name',
                defaultContent: '',
                orderable: false,
                searchable: true, 
                visible: true,
                mRender: function (row, type, full) {
                    ...
                    if(full.data.communitydocument.fields.visible){
                        let name = full.data.communitydocument.fields.name;
                        if (name.length > 30)
                        {
                            name = name.substring(0, 27) + "...";
                        }
                        links+='<a href="/private-media/'+ full.data.communitydocument.fields._file+'" target="_blank"><p>' + name + '</p></a>' ;
                    }else{
        ...
        column_description: function () {
            return {
                // 4. Description
                data: 'data.data.communitydocument.fields.description',
                defaultContent: '',
                orderable: false,
                searchable: true, 
                visible: true,
                render: function (row, type, full) {
                    let value = full.data.communitydocument.fields.description;
                    let result = helpers.dtPopover(value, 30, 'hover');
        ...
        datatable_options: function () {
            ...
            return {
                ...
                serverSide: true,
                ajax: {
                    url: api_endpoints.lookup_history_community_document(this.documentId)+"?format=datatables",
                    dataSrc: 'data',
                },
                buttons: [
                    {
