<template lang="html">
    <div id="occ_documents">
        <FormSection :formCollapse="false" label="Documents" Index="documents">
            <alert type="warning"><i class="bi bi-ban fs-6 fw-bold me-2"></i>Do not upload Management or Recovery Plans
                here</alert>
        ...
        <DocumentDetail ref="document_detail" @refreshFromResponse="refreshFromResponse" :url="occ_document_url"
            :is_internal="is_internal"></DocumentDetail>
        <div v-if="occurenceDocumentHistoryId">
            <OccurenceDocumentHistory ref="occ_document_history" :key="occurenceDocumentHistoryId"
                :document-id="occurenceDocumentHistoryId" :occurrence-id="occurrence_obj.id" />
        </div>
    </div>
</template>
<script>
...
export default {
    ...
    data: function () {
        ...
        return {
            ...
            documents_options: {
                ...
                ajax: {
                    "url": helpers.add_endpoint_json(api_endpoints.occurrence, vm.occurrence_obj.id + '/documents'),
                    "dataSrc": ''
                },
                order: [],
    ...
    methods: {
        ...
        editDocument: function (id) {
            ...
            fetch(helpers.add_endpoint_json(api_endpoints.occurrence_documents, id)).then(async (response) => {
        ...
        discardDocument: function (id) {
            ...
            swal.fire({
                ...
                if (result.isConfirmed) {
                    fetch(helpers.add_endpoint_json(api_endpoints.occurrence_documents, id + '/discard'), {
                        method: 'PATCH',
        ...
        reinstateDocument: function (id) {
            ...
            swal.fire({
                ...
                if (result.isConfirmed) {
                    fetch(helpers.add_endpoint_json(api_endpoints.occurrence_documents, id + '/reinstate'), {
                        method: 'PATCH',
                        headers: { 'Content-Type': 'application/json', }
                    })
                        .then((response) => {
