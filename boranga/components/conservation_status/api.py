...
class GetCSProfileDict(views.APIView):
    def get(self, request, format=None):
        ...
        if group_type and (
            ...
            or group_type == GroupType.GROUP_TYPE_FAUNA
        ):
            # action is used to filter conservation list for WA or all
            if action == "view":
                lists = ConservationList.objects.filter(applies_to_species=True)
            else:
                lists = ConservationList.objects.filter(
                    applies_to_species=True, applies_to_wa=True
                )
            ...
            if action == "view":
                ...
                lists = ConservationList.objects.filter(
                    applies_to_communities=True, applies_to_wa=True
                )
        if lists:
            for option in lists:
...
class SpeciesConservationStatusFilterBackend(DatatablesFilterBackend):
    def filter_queryset(self, request, queryset, view):
        ...
        if queryset.model is ConservationStatus:
            if (
                filter_conservation_list
                and not filter_conservation_list.lower() == "all"
            ):
...
class ConservationStatusViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if is_internal(self.request):  # user.is_authenticated():
            ...
            return qs
        logger.warn(
            f"User is neither customer nor internal user: {user.get_full_name()} <{user.email}>"
        )
            ...
            with transaction.atomic():
                ...
                if instance.processing_status == "approved":
                    ...
                    if (
                        ...
                        instance.application_type.name == GroupType.GROUP_TYPE_COMMUNITY
                    ):
                        serializer = SaveCommunityConservationStatusSerializer(
                            instance, data=request_data, partial=True
                        )
    ...
    def amendment_request(self, request, *args, **kwargs):
        try:
            ...
            serializer = ConservationStatusAmendmentRequestDisplaySerializer(
                qs, many=True
            )
            return Response(serializer.data)
        except serializers.ValidationError:
