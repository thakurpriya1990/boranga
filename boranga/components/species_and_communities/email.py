...
class UserCreateSpeciesSendNotificationEmail(TemplateEmailBase):
    subject = '{} - Confirmation - Species submitted.'.format(settings.DEP_NAME)
...
class UserCreateCommunitySendNotificationEmail(TemplateEmailBase):
    subject = '{} - Confirmation - Community submitted.'.format(settings.DEP_NAME)
    html_template = 'boranga/emails/send_user_create_notification.html'
    txt_template = 'boranga/emails/send_user_create_notification.txt'
...
def send_species_create_email_notification(request, species_proposal):
    ...
    url = request.build_absolute_uri(reverse('internal-species-detail',kwargs={'species_proposal_pk': species_proposal.id}))
    # add the extra query params as need to load the species detail page
    url = url+'?group_type_name={}&action=view'.format(species_proposal.group_type.name)
...
def send_user_species_create_email_notification(request, species_proposal):
    ...
    msg = email.send(
        EmailUser.objects.get(id=species_proposal.submitter).email,cc=all_ccs, 
        context=context
        )
    sender = request.user if request else settings.DEFAULT_FROM_EMAIL
    _log_species_email(msg, species_proposal, sender=sender)
...
def send_community_create_email_notification(request, community_proposal):
    ...
    url = request.build_absolute_uri(reverse('internal-community-detail',kwargs={'community_proposal_pk': community_proposal.id}))
    # add the extra query params as need to load the species detail page
    url = url+'?group_type_name={}&action=view'.format(community_proposal.group_type.name)
    if "-internal" not in url:
        # add it. This email is for internal staff 
        url = '-internal.{}'.format(settings.SITE_DOMAIN).join(url.split('.' + settings.SITE_DOMAIN))
...
def send_user_community_create_email_notification(request, community_proposal):
    ...
    msg = email.send(
        EmailUser.objects.get(id=community_proposal.submitter).email,cc=all_ccs, 
        context=context
        )
    sender = request.user if request else settings.DEFAULT_FROM_EMAIL
    _log_community_email(msg, community_proposal, sender=sender)
